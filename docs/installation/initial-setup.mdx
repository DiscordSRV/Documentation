...
Go to your Discord settings by clicking on the `User Settings` wheel on the bottom left of your Discord.
...
![open server settings](/assets/server_settings.png)

:::

:::info 

Go to the `Roles` tab and create a new role. We've named ours `Bot`. Add the `Administrator` permission (or [permissions listed below](#server-permissions)) to the new role.
...
![Add Bot-rule to bot](/assets/add_role.gif)
