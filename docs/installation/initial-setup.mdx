...
Uncheck `PUBLIC BOT` so only you can invite the bot to the servers you designate.

![public bot unchecked](/assets/public_bot_unchecked.png)
...
Press `Reset Token` then `Copy`. Keep `PUBLIC BOT` unchecked so only you can invite the bot to the servers you designate.

![get bot token](/assets/get_token.png)
