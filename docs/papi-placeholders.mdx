...
<details>
<summary>Some placeholders require [`EnablePresenceInformation`](https://config.discordsrv.com/config/EnablePresenceInformation) to be enabled!</summary>
...
Please note that to use these placeholders you need to have the Presence Intent enabled on your bot using the Discord developer portal. You enable this intent the same way you enabled the [`Server Members Intent`](/installation/initial-setup#enable-the-privileged-gateway-intents-options).
...
%discordsrv_role_<roleid>_name%
%discordsrv_role_<roleid>_color_hex%
%discordsrv_role_<roleid>_color_code%
```
